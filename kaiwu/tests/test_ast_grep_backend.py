...
class UserService:
    def login(self, token):
        ...
        self.audit_login(token)

    def audit_login(self, token):
        return token
...
def test_ast_grep_extracts_python_functions_and_calls():
    parser = AstGrepParser()

    tree = parser.parse_bytes(PYTHON_SOURCE, "python")
    assert tree is not None

    functions = parser.extract_functions(tree, PYTHON_SOURCE, "python")
    function_names = {func["name"] for func in functions}

    assert "UserService.login" in function_names
    assert "UserService.audit_login" in function_names
    assert "validate_token" in function_names

    login = next(func for func in functions if func["name"] == "UserService.login")
    assert login["params"] == ["self", "token"]

    calls = parser.extract_calls(tree, PYTHON_SOURCE, "python")
    call_pairs = {(call["in_function"], call["name"]) for call in calls}

    assert ("UserService.login", "validate_token") in call_pairs
    assert ("UserService.login", "audit_login") in call_pairs


def test_ast_grep_extracts_go_functions_and_calls():
    parser = AstGrepParser()
...
def test_graph_builder_uses_ast_grep_backend_for_go_files(tmp_path, monkeypatch):
    ...
    assert isinstance(builder._parser, AstGrepParser)
